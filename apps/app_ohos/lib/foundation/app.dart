...
import 'package:venera/bridge/ohos_platform.dart';
...
class _App {
  ...
  bool get isOhos => _isOhosPlatform();
  ...
  Future<void> init() async {
    if (isOhos) {
      await OhosPlatform.init();
    }
    cachePath = (await OhosPathProvider.getApplicationCacheDirectory()).path;
    dataPath = (await OhosPathProvider.getApplicationSupportDirectory()).path;
    if (isAndroid) {
      externalStoragePath = (await OhosPathProvider.getExternalStorageDirectory())!.path;
    }
    isInitialized = true;
  }
