...
class _LoginPageState extends State<_LoginPage> {
            ...
            child: Column(
              ...
              children: [
                ...
                if (widget.config.registerWebsite != null)
                  TextButton(
                    onPressed: () =>
                        OhosUrlLauncher.launchUrlString(widget.config.registerWebsite!),
                    child: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
  ...
  void loginWithWebview() async {
    ...
    void validate(AppWebviewState webviewState) async {
      if (widget.config.checkLoginStatus != null &&
          widget.config.checkLoginStatus!(url, title)) {
        var cookiesMap = await webviewState.getCookies(url);
        ...
        widget.source.data['_localStorage'] = <String, dynamic>{};
  ...
  void loginWithWebview2() async {
    ...
    void validate(DesktopWebview w) async {
      if (widget.config.checkLoginStatus != null &&
          widget.config.checkLoginStatus!(url, title)) {
        var cookiesMap = await w.getCookies(url);
        var cookies = <io.Cookie>[];
        cookiesMap.forEach((key, value) {
          cookies.add(io.Cookie(key, value));
        });
        ...
        var localStorageJson = await w.evaluateJavascript(
